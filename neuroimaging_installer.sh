...
install_spm() {
    ...
    local spm_base_dir="${INSTALL_DIR}/matlab"
    local temp_file="/tmp/spm_${SPM_VERSION}.zip"
    local spm_url="${DOWNLOAD_URLS[spm]}"

    print_message "Downloading SPM..."
    wget --progress=bar:force "$spm_url" -O "$temp_file"
    mkdir -p "$spm_base_dir"

    # Unzip
    print_message "Extracting SPM..."
    unzip -q -o "$temp_file" -d "$spm_base_dir"

    # Rename extracted folder to 'spm' for consistency
    if [ -d "$spm_base_dir/spm12" ]; then
        mv "$spm_base_dir/spm12" "$spm_base_dir/spm"
    fi

    print_success "SPM installed in $spm_base_dir/spm"
    rm -f "$temp_file"
}
...
install_conn() {
    ...
    local conn_base_dir="${INSTALL_DIR}/matlab"
    local conn_url="${DOWNLOAD_URLS[conn]}"
    
    print_message "Downloading CONN..."
    wget --progress=bar:force "$conn_url" -O /tmp/conn.zip
    
    print_message "Extracting CONN..."
    mkdir -p "$conn_base_dir"
    unzip /tmp/conn.zip -d "$conn_base_dir"
    
    unzip -q -o /tmp/conn.zip -d "$conn_base_dir"

    print_success "CONN installed in $conn_base_dir/conn"
    rm -f /tmp/conn.zip
}
...
configure_matlab_startup() {
    print_header "MATLAB STARTUP CONFIGURATION"

    # Standard path for startup.m
    local matlab_docs="$HOME/Documents/MATLAB"
    local startup_file="$matlab_docs/startup.m"
    
    # Installation paths defined in the script
    local spm_path="${INSTALL_DIR}/matlab/spm"
    local conn_path="${INSTALL_DIR}/matlab/conn"

    mkdir -p "$matlab_docs"

    print_message "Creating startup.m file in $startup_file..."

    cat <<EOF > "$startup_file"
% =========================================================================
% AUTOMATIC CONFIGURATION GENERATED BY NEUROIMAGING INSTALLER
% Data: $(date)
% =========================================================================

disp('Initializing Neuroimaging environment...');

% SPM configuration
if exist('$spm_path', 'dir')
    addpath('$spm_path');
    disp(' - SPM added to path');
end

% CONN configuration
if exist('$conn_path', 'dir')
    addpath('$conn_path');
    disp(' - CONN added to path');
end

disp('MATLAB environment ready.');
EOF

    # Fix permissions (just for safety)
    chmod 644 "$startup_file"
    
    print_success "startup.m file created successfully."
    print_message "MATLAB will automatically load these paths on next startup."
}
...
main() {
    ...
    if [ "${INSTALL_SOFTWARE[spm]}" = true ] || [ "${INSTALL_SOFTWARE[conn]}" = true ]; then
        configure_matlab_startup
    fi
